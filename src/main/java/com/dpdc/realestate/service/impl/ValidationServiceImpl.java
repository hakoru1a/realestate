...
public class ValidationServiceImpl implements ValidationService {
    ...
    public void validateCredential(CredentialRegister credential, boolean isCustomer) {
        ...
        if (customerService.isExistByUsername(credential.getUsername(), isCustomer) ||
            ...
            boolean a = customerService.isExistByUsername(credential.getUsername(), isCustomer);
            boolean b = userService.isExistByUsername(credential.getUsername(), !isCustomer);
    ...
    @Override
    public void validateCredential(Integer id, CredentialRegister credential, boolean isCustomer) {
        Map<String, String> errors = new HashMap<>();

        if (customerService.isExistByPhone(id, credential.getPhone(), isCustomer) ||
                userService.isExistByPhone(id, credential.getPhone(), !isCustomer)) {
            errors.put("phone", env.getProperty("api.notify.already_exist"));
        }
        if (customerService.isExistByUsername(id, credential.getUsername(), isCustomer) ||
                userService.isExistByUsername(id, credential.getUsername(), !isCustomer)) {
            boolean a = customerService.isExistByUsername(credential.getUsername(), isCustomer);
            boolean b = userService.isExistByUsername(credential.getUsername(), !isCustomer);
            errors.put("username", env.getProperty("api.notify.already_exist"));
        }
        if (customerService.isExitsByEmail(id, credential.getEmail(), isCustomer) ||
                userService.isExitsByEmail(id, credential.getEmail(), !isCustomer)) {
            errors.put("email", env.getProperty("api.notify.already_exist"));
        }
